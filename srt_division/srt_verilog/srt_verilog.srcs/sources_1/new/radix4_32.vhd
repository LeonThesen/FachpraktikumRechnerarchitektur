...
architecture behav of radix4_32 is
    type state_type is (init, norm, norm2, div, d);
...
begin
    ...
    rem_add <= (left_half_remainder sll 1) + (unsigned('0' & std_logic_vector(divisor_r)) sll 1);
        ...
        if rst = '1' then
            ...
            case state is
                ...
                when norm2 =>
                    ...
                    --remainder_r <= remainder_r sll to_integer(unsigned(j)); -- TODO: this was j(0) before
                ...
                when div =>
                    ...
                    case selected_quotient is 
                        when -2 =>
                            ...
                            quotient <= std_logic_vector(unsigned(quotient) + (to_unsigned(2, 32) sll shift_amount));
                        when -1 =>
                            remainder_r(width+2 downto (width+1)/2+2) <= rem_sub(width_h+2 downto 2);
                            quotient <= std_logic_vector(unsigned(quotient) + (to_unsigned(1, 32) sll shift_amount));
                        when 1 =>
                ...
                when d =>
                    ...
                    if remainder_r(width + 2) = '1' then 
                        remainder <= std_logic_vector(remainder_correction srl (to_integer(j) + 1)); -- FIXME: [Synth 8-690] width mismatch in assignment; target has 32 bits, source has 18 bits ["/home/leon/source/srt_verilog/srt_verilog.srcs/sources_1/new/radix4_32.vhd":280]
                        quotient <= std_logic_vector(signed(quotient) - 1);
                        report "remainder_r is " & integer'image(to_integer(unsigned(remainder_r)));
                        ...
                        remainder <= std_logic_vector(remainder_r(width + 1 downto (width + 1) / 2) srl (to_integer(j) + 1) );
                        report "remainder_r is " & integer'image(to_integer(signed(remainder_r)));
                        report "remainder is positive so shift remainder";
                        --report "remainder = " & integer'image(to_integer(remainder_r(width + 1 downto (width + 1) / 2) srl (j + 1)))) & ", quotient = " & integer'image(to_integer(signed(quotient)));
                    end if;
                    done <= '1';
            end case;
        end if;
