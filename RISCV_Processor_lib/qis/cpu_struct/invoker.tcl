...
if {[is_project_open]} {
   if {[string compare $quartus(project) "cpu"]} {
      puts "Project cpu is not open"
      set make_assignments 0
   }
} else {
   # Only open if not already open
   if {[project_exists cpu]} {
      project_open -revision cpu cpu
   } else {
      project_new -revision cpu cpu
   }
...
# Make assignments
if {$make_assignments} {
   set_global_assignment -name COMPILER_SETTINGS cpu
   set_global_assignment -name SIMULATOR_SETTINGS cpu
   set_global_assignment -name SOFTWARE_SETTINGS cpu
   set_global_assignment -name FMAX_REQUIREMENT 50MHz
   set_global_assignment -name FAMILY "cyclone v"
   set_global_assignment -name TOP_LEVEL_ENTITY cpu
   set_global_assignment -name DEVICE 5cgxfc5c6f27c7
   set_global_assignment -name USE_COMPILER_SETTINGS cpu
	...
	set_global_assignment -name VHDL_FILE "/u/home/clab/st161569/FachpraktikumRechnerarchitektur/RISCV_Processor_lib/hdl/bpb_entity.vhd" 
	set_global_assignment -name VHDL_FILE "/u/home/clab/st161569/FachpraktikumRechnerarchitektur/RISCV_Processor_lib/hdl/bpb_behav.vhd" 
